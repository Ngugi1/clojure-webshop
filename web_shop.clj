...
(defn- buy-product [store-id product-id n]
  ...
  (dosync (alter (nth stock product-id) ; We update the refs by row.
    (fn [old-stock]
      (update old-stock store-id
...
(defn buy-products [store-id product-ids-and-number]
 ; (doall (pmap buy-product product-ids-and-number))
...
(defn- process-customer [customer]
  ...
  (dosync (let [product-ids-and-number
